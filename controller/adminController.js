...
const adminHome = async (req, res) => {
  try {
    res.render("admin/adminHome")
      ...
      res.render('admin/adminHome', { data, today, yesterday, last7days, last30days, lastYear });
  } catch (err) {
      console.log(err.message);
  }
};
...
const adminBlockUnblock = async (req, res) => {
    try {
      ...
      await user.save();
      res.json({ status: true, message: user.isBlocked ? "User blocked successfully" : "User unblocked successfully" });
    } catch (err) {
      console.error(err);
  ...
  //pagination endss    
    ...
    next(error)
      ...
      try {
        ...
        const productIds = Array.from(new Set(orderData.flatMap((order) => order.items.map((items) => items.product))));
